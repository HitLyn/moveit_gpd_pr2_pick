...
{
  arm_ = new moveit::planning_interface::MoveGroupInterface("left_arm");
  hand_ = new moveit::planning_interface::MoveGroupInterface("left_gripper");
...
void PR2_Shadow_Grasp::planPickCallback(const geometry_msgs::PoseStamped& msg)
{
  ...
  br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "odom_combined", "grasp"));
  ...
  if(result.error_code.val == 1)
  {
    ...
    if (scanKeyboard() == 112)
    {
      ...
      moveit::planning_interface::MoveGroupInterface hand("left_gripper");
...
std::vector<moveit_msgs::Grasp> PR2_Shadow_Grasp::generateGrasp(const geometry_msgs::PoseStamped pose_)
{
  ...
  jointValuesToJointTrajectory(hand_->getNamedTargetValues("closed"), ros::Duration(2.0), grasp.grasp_posture);
...
void PR2_Shadow_Grasp::executePick()
{
  ...
  if(current_trajectories_.size() > 0)
  {
    ...
    attached_object.link_name = "l_gripper_tool_frame";
